...
its own message above the grid, and the edited values stay where they are. The reasoning is
in [ADR-0015](docs/adr/0015-editace-radku.md) (Czech).
...
| `403` on `/rows/update` | Editing is off (`AllowUpdate`, `AllowDelete`), or the table isn't in `EditableTables`. |

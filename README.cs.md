...
To je všechno — ostatní balíčky (`DbsViewer.Abstractions`, `EfCore`, `Relational`,
`SqlServer`, `Sqlite`, `Analysis`) se přitáhnou jako závislosti. Blazor UI je vložené
přímo v `DbsViewer.Server`, takže žádný další zdroj souborů nepotřebuješ.

Chceš-li nejnovější vývojovou verzi, sáhni po předběžném vydání z `main`:

```bash
dotnet add package DbsViewer.Server --prerelease
```

Sestavení ze zdrojů popisuje kapitola [Vývoj](#vývoj) — pro běžné použití ho nepotřebuješ.

### Krok 2: zapoj do `Program.cs`
...
Rozbalení uzlu na všechny sloupce s rozvržením nehýbe — o tom, kam která tabulka patří,
rozhodují sbalené výšky, takže rozbalený uzel jen roztlačí svůj sloupec a to, co je pod
ním. Podrobněji v [ADR-0016](docs/adr/0016-rozvrzeni-velkeho-schematu.md).
...
Balíčky ze zdrojů — pro vyzkoušení změny v cizí aplikaci dřív, než vyjde na nuget.org:
...
Cílová aplikace pak potřebuje `artifacts/packages` jako zdroj balíčků — buď
`dotnet add package DbsViewer.Server --source <cesta>`, nebo `NuGet.config` vedle
jejího `.csproj`:
...
| `samples/DbsViewer.SampleMigrations` | Model se skutečnými EF migracemi — kvůli historii schématu |
...
| — | Publikování na nuget.org | ✅ hotovo |
